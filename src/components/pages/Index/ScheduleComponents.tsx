...
export const TableColumn: React.FC = (props) => {
  return <div className={styles.TableColumn}>{props.children}</div>;
};

export const TableRow: React.FC = (props) => {
  return <div className={styles.TableRow}>{props.children}</div>;
};

export const TableRowSeparator: React.FC = () => {
  return <div className={styles.TableRowSeparator} />;
};

export const RequestProposalButton: React.FC<IRequestProposalButton> = (
  props
) => {
  if (!props.startTime) {
    return <></>;
  ...
  const onClick = () => {
    window.location.href = getHref(
      `/tarjouspyynto?date=${props.startTime}`
    ).toString();
  };

  return (
    <button className={styles.RequestProposalButton} onClick={onClick}>
      Pyydä tarjous
    </button>
  );
};
...
export const DisabledRequestProposalButton: React.FC = () => {
  return (
    <button
      className={addStylesToClass(styles.RequestProposalButton, [
        styles.disabled,
      ])}
    >
      Kuljetus täysi
