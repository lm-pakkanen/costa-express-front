...
const useContactForm = (): IContactFormState => {
  ...
  const onFormChange = (e: React.FormEvent<HTMLInputElement>) => {
    ...
    setFormData({
      ...formData,
      [e.currentTarget.name]: { value: e.currentTarget.value, error: null },
    });
  };
  ...
  const setErrors = (errors: { [key: string]: string }) => {
    ...
    const deliveryAddressZipAndCityError =
      errors.deliveryAddressZipAndCity ?? null;
  ...
  const onFormSubmit = async (e: React.FormEvent<HTMLInputElement>) => {
    ...
    if (!templateID) {
      const errMessage =
        "Sähköpostin lähettäminen ei onnistunut. Yritäthän myöhemmin uudelleen!";
    ...
    try {
      ...
      await sendEmail(templateID, messageVariables);

      setFormData(initialFormData);
      setFormAlert(
        "Sähköpostisi on lähetetty! Vastaamme viestiin mahdollisimman pian."
      );
  ...
  const validateFields = (data: IContactFormState) => {
    ...
    const emailValid = Validator.validateContactFormEmail(
      data.sender.emailAddress.value
    );

    const firstNameValid = Validator.validateContactFormName(
      data.sender.firstName.value
    );

    const lastNameValid = Validator.validateContactFormName(
      data.sender.lastName.value
    );

    const pickupStreetValid = Validator.validateAddressStreet(
      data.pickupAddress.street.value
    );

    const pickupZipAndCityValid = Validator.validateAddressZip(
      data.pickupAddress.zipAndCity.value
    );

    const pickupCountryValid = Validator.validateAddressCountry(
      data.pickupAddress.country.value
    );

    const pickupPhoneValid = Validator.validatePhoneNumber(
      data.pickupPhone.value
    );

    const deliveryStreetValid = Validator.validateAddressStreet(
      data.deliveryAddress.street.value
    );

    const deliveryZipAndCityValid = Validator.validateAddressZip(
      data.deliveryAddress.zipAndCity.value
    );

    const deliveryCountryValid = Validator.validateAddressCountry(
      data.deliveryAddress.country.value
    );

    const deliveryPhoneValid = Validator.validatePhoneNumber(
      data.deliveryPhone.value
    );

    const cargoMessageValid = Validator.validateContactFormCargoInformation(
      data.cargoDescription.value
    );

    const messageValid = Validator.validateContactFormMessage(
    ...
    if (
      !(typeof pickupZipAndCityValid === "boolean" && pickupZipAndCityValid)
    ) {
      errors = updateErrors(
        errors,
        "pickupAddressZipAndCity",
        pickupZipAndCityValid
      );
      hasError = true;
    }
    ...
    if (
      !(typeof deliveryZipAndCityValid === "boolean" && deliveryZipAndCityValid)
    ) {
      errors = updateErrors(
        errors,
        "deliveryAddressZipAndCity",
        deliveryZipAndCityValid
      );
      hasError = true;
    }
    ...
    return hasError
      ? setFormError("Jossain kentässä on virheellistä tietoa.")
      : true;
  };

  const sendEmail = async (
    templateID: string,
    variables: { [key: string]: null | string }
  ) => {
    try {
      const endpoint = `${
        constants.BASE_API_URI
      }/emails/templates/${encodeURIComponent(templateID)}/send`;

      const result: AxiosResponse = await axios.post(endpoint, variables);

      if (result.status !== 200) {
        throw new Error("Viestiä ei voitu lähettää.");
      }
    } catch (err) {
      console.error(err);
    }
